...
void main() {
  ...
  group("signin", () {
    Credential credential = Credential(
        phone: "9876543210", type: UserType.patient, name: "name-name");
    test("when the status code is not 200", () async {
      when(_mock.post(any, body: anyNamed('body')))
          .thenAnswer((_) async => http.Response('{}', 404));
      dynamic result = await _api.signIn(credential);
      expect(result, isA<ErrorResult>());
    });
    test("when the status code is 200 but response is null", () async {
      when(_mock.post(any, body: anyNamed('body'))).thenAnswer(
          (_) async => http.Response('{"message":"Email is not valid"}', 200));
      dynamic result = await _api.signIn(credential);
    ...
    test("when the status code is 200 and working", () async {
      when(_mock.post(any, body: anyNamed('body')))
          .thenAnswer((_) async => http.Response('{"authToken":"abbc"}', 200));
      dynamic result = await _api.signIn(credential);
